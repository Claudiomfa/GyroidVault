const App = {
  ...
  async init() {
    ...
    if (this.publicConfig.require_login_to_view && !this.currentUser) {
      document.getElementById('app').innerHTML = '<div style="height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;background:var(--bg-color)"><h1 style="margin-bottom:20px">RedPOISON 3d</h1><button class="btn btn-primary btn-lg" onclick="App.showLogin()">Faça login para aceder ao RedPOISON 3d.</button></div>';
      this.showLogin();
      return;
    }
  ...
  async openWhatsNew() {
    try {
      ...
      this.openModal('Welcome to RedPOISON 3d 2.0', UI.whatsNewModal('2.0.0', this.cachedReleaseNotes), 'modal-lg');
    } catch (e) {
      this.openModal('Welcome to RedPOISON 3d 2.0', UI.whatsNewModal('2.0.0', []), 'modal-lg');
    }
  },
  ...
  closeModal() {
    ...
    const isLocalhost = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
    if (!isLocalhost && (title.includes('RedPOISON 3d 2.0') || title.includes('RedPOISON 3d v2'))) {
      localStorage.setItem('gv_last_seen_version', '2.0.0');
    }
      ...
      if (tab === 'account') {
        ...
        content.innerHTML = `
          <div class="glass-panel">
            ...
            <div class="panel-body">
              <div class="form-group" style="margin-bottom:0">
                <p style="font-size: 0.9rem; color: var(--text-muted); margin-bottom: 20px; line-height: 1.5;">Gere uma chave API para permitir que ferramentas externas (como os scripts de pós-processamento do OrcaSlicer) interajam de forma segura com a sua conta RedPOISON 3d. Mantenha essa chave em segredo.</p>
