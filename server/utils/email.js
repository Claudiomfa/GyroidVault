...
async function sendResetEmail(email, token, originOrReq) {
  ...
  const from = get('SELECT value FROM system_settings WHERE key="smtp_from"')?.value || 'RedPOISON 3d <noreply@redpoison.pt>';
  ...
  await transporter.sendMail({
    from,
    to: email,
    subject: 'Password Reset - RedPOISON 3d',
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 8px;">
        <h1 style="color: #8b5cf6;">RedPOISON 3d</h1>
        <h2 style="color: #1e293b;">Password Reset Request</h2>
        <p style="color: #475569; line-height: 1.6;">A password reset was requested for your RedPOISON 3d account. Click the button below to choose a new password.</p>
...
async function sendTestEmail(email) {
  ...
  await transporter.sendMail({
    from,
    to: email,
    subject: 'RedPOISON 3d SMTP Test Successful',
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 8px;">
        <h1 style="color: #00d4ff;">RedPOISON 3d</h1>
        <h2 style="color: #1e293b;">SMTP Configuration Test</h2>
        <p style="color: #475569; line-height: 1.6;">If you are reading this email, your SMTP settings in RedPOISON 3d have been configured correctly!</p>
